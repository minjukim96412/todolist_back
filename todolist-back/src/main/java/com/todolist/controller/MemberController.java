...
public class MemberController {
    ...
    @ApiResponses({
        @ApiResponse(responseCode = "200", description = "업데이트 성공", content = @Content(schema = @Schema(implementation = MemberEntity.class))),
        @ApiResponse(responseCode = "400", description = "잘못된 요청")
    })
    @PatchMapping("/update-nickname/{memId}") // PATCH 메서드로 변경
    public ResponseEntity<Map<String, Object>> updateNickname(
            @PathVariable Integer memId,
            @io.swagger.v3.oas.annotations.parameters.RequestBody(description = "닉네임 업데이트 데이터", required = true, content = @Content(schema = @Schema(type = "object", example = "{\"nickname\": \"newNickname\"}")))
            @RequestBody Map<String, String> request) {
        ...
        MemberEntity updatedMember = memberService.updateNickname(memId, newNickname);
